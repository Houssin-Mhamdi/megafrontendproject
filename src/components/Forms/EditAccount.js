...
import { getSingleAcountAction, updateAcountAction } from "../../redux/slice/accounts/accountSlice";
const EditAccount = () => {
  const { id } = useParams()
  ...
  const { account, loading, error, isUpdatae } = useSelector((state) => state?.accounts)
  console.log(account);
  console.log(isUpdatae);
  ...
  const onSubmit = (e) => {
    ...
    dispatch(updateAcountAction({ ...transaction, id }))
  };
