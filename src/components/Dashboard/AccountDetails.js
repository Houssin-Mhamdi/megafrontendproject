...
import { getSingleAcountAction } from "../../redux/slice/accounts/accountSlice";
...
const AccountDetails = () => {
  ...
  useEffect(() => {
    dispatch(getSingleAcountAction(id));
  }, [id])

  //get data from the store 
  const { account, error, loading } = useSelector((state) => state?.accounts)
  const transactions = account?.data?.transactions;
  const totalIncome = transactions?.filter((transaction) => transaction?.transactionType === 'Income').reduce((acc, curr) => {
    return acc + curr.amount
  }, 0)

  const totalExpenses = transactions?.filter((transaction) => transaction?.transactionType === 'Expenses').reduce((acc, curr) => {
    return acc + curr.amount
  }, 0)
          ...
          <div className="text-center">
            ...
            <div className="flex flex-wrap justify-center -mx-4">
              <div className="w-full md:w-1/3 lg:w-1/4 px-4 mb-8 lg:mb-0">
                {loading ? <h2 className="text-center text-green-600 text-2xl mt-5">...</h2> : <h2 className="mb-2 text-4xl md:text-5xl text-red-600 font-bold tracking-tighter">
                  ${totalExpenses}
                </h2>}
              ...
              <div className="w-full md:w-1/3 lg:w-1/4 px-4 mb-8 lg:mb-0">
                {loading ? <h2 className="text-center text-green-600 text-2xl mt-5">...</h2> : <h2 className="mb-2 text-4xl md:text-5xl text-coolGray-900 font-bold tracking-tighter">
                  ${totalIncome}
                </h2>}
